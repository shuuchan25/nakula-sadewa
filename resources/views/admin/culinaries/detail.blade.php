                ...
                <form action="/admin/culinaries/{{ $culinary->slug }}" method="GET" id="search-form" class="w-100">
                    <div class="item-filters">
                        <div class="search">
                            ...
                            <input type="text" name="search" class="" id="search-input" placeholder="Cari Menu..." value="{{ request('search') }}">
                        </div>
                        <div class="select-box">
                            <select name="menu_category_id">
                                <option value="">Kategori</option>
                                @foreach ($menuCategories as $menuCategory)
                                    <option value="{{ $menuCategory->id }}" {{ request('menu_category_id') == $menuCategory->id ? 'selected' : '' }}>
                                        {{ $menuCategory->name }}
                                    </option>
                                @endforeach
                            </select>
